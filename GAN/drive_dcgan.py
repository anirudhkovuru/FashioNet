...
class DCGAN():
    ...
    def load_shirts(self):
        names = [f for f in listdir("drive/pics/images64.zip(Unzipped Files)/images64/") if isfile(join("drive/pics/images64.zip(Unzipped Files)/images64/", f))]
        return names
    ...
    def load_batch(self, idx, names):
        ...
        for i in idx:
            try:
                img = Image.open("drive/pics/images64.zip(Unzipped Files)/images64/"+names[i])
    ...
    def save_imgs(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("drive/app/images/testshirt_%d.png" % epoch)
        plt.close()
...
if __name__ == '__main__':
    ...
    dcgan.generator = load_model("drive/app/saved-models/generator-model.h5")
    print("Generator loaded.")
    print("Loading discriminator...")
    dcgan.discriminator = load_model("drive/app/saved-models/discriminator-model.h5")
    print("Discriminator loaded.")
    # Train the DCGAN
    dcgan.train(epochs=6000, batch_size=5, save_interval=20)
    # Save the generator-discriminator combined model
    dcgan.generator.save("drive/app/saved-models/generator-model.h5")
    dcgan.discriminator.save("drive/app/saved-models/discriminator-model.h5")
